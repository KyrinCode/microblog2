...
async function getUserPosts(id){
  showLoading(true);
  userPosts = await microblog2.followPosts(id, 1000000);
  showLoading(false);
...
async function getPosts() {
  showLoading(true);
  posts = await microblog2.posts(1000000);
  showLoading(false);
  setMessages(posts);
}
...
async function getTimeLine() {
  showLoading(true);
  timeline = await microblog2.timeline(1000000);
  showLoading(false);
...
async function getFollows() {
  showLoading(true);
  follows = await microblog2.followsInfo();
  showLoading(false);
  setFollows(follows);
}
...
async function modifyName(message, password) {
  showLoading(true);
  try {
    await microblog2.set_name(password, message);
    showLoading(false);
    $(".dialog-bg").hide();
    resetDialog();
    getInfo();
...
async function follow(message, password) {
  ...
  try {
    await microblog2.follow(password, Principal.fromText(message));
    showLoading(false);
    $(".dialog-bg").hide();
    resetDialog();
    getFollows();
...
async function modifyPassword(message, password) {
  showLoading(true);
  try {
    await microblog2.modifyPassword(password, message);
    showLoading(false);
    $(".dialog-bg").hide();
    resetDialog();
  } catch (error) {
...
async function post(message, password) {
  showLoading(true);
  try {
    await microblog2.post(password, message);
    showLoading(false);
    $(".dialog-bg").hide();
    resetDialog();
    getPosts();
